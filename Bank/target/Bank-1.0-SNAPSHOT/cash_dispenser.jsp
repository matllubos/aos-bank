<%-- 
    Document   : cash_dispenser
    Created on : 14.12.2011, 15:30:42
    Author     : lubos
--%>
...
<html>
    ...
    <body>
        ...
        <form action="cash_dispenser.jsp?accountNumber=<%=request.getParameter("accountNumber")%>&currency=<%=request.getParameter("currency")%>" method="POST">
            <%
                if (request.getParameter("amount")!= null) {
                    try {
                        Client_Service service = new Client_Service();
                        client.Client port = service.getClientPort();
                        java.math.BigDecimal result = port.withdrawMoney(Integer.parseInt(request.getParameter("accountNumber")), new BigDecimal(request.getParameter("amount")), request.getParameter("currency"));;

                        out.print("<p class=\"sucess\">Prosím vyberte peníze: "+result+"</p>");
                    } catch (Exception ex) {
                        System.out.println("<p class=\"error\">Na účtu není dostatek peněz</p>");
                    }
                }
            %>
